  ...
  parseObject(dataTable) {
    ...
    for (const [path, value] of Object.entries(rawContent)) {
      if (value.includes('_AGO_')) {
        ...
        _.set(content, path, Math.trunc((this.props.now - timeAgo) / 1000));
      }
      else if (value.includes('_AFTER_')) {
        // format: "_5m_AFTER_"
        const timeAfter = ms(value.split('_')[1]);

        _.set(content, path, Math.trunc((this.props.now + timeAfter) / 1000));
      }
      else {
